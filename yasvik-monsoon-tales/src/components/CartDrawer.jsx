                ...
                items.map((item) => (
                  <div key={item.key} className="flex gap-3 rounded-xl bg-[#FBF7EA] p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {item.variant && <p className="mt-0.5 font-inter text-[11px] text-[#96947B]">{item.variant}{item.pack_kg ? ` · ${item.pack_kg}kg pack` : ''}</p>}
                      {item.unit && !item.variant && <p className="mt-0.5 font-inter text-[11px] text-[#96947B]">{item.unit}</p>}
                      {getLineItemStockKg(item) > 0 && <p className="mt-0.5 font-inter text-[10px] text-[#96947B]">Bulk stock use: {getLineItemStockKg(item)}kg</p>}
