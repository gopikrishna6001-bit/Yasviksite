...
export default function FarmerSpotlight({ product }) {
  ...
  const partnerName = person?.name || 'Sourcing journey';
  const location =
      ...
      <div className="p-5 flex flex-col md:flex-row gap-5">
        <div className="md:w-48 flex-shrink-0">
          <div className="aspect-[4/5] w-full rounded-xl overflow-hidden bg-rain-mist border border-rain-cloud/10">
            {portrait ? (
              <img src={portrait} alt={partnerName} className="w-full h-full object-cover" />
            ) : (
        ...
        <div className="flex-1 min-w-0">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-rain-cloud/12 bg-rain-mist/35 p-3">
              <p className="font-inter text-[10px] uppercase tracking-[0.18em] text-rain-cloud/45">Producer / Regional Partner</p>
              <p className="mt-1 font-cormorant text-2xl leading-none text-rain-cloud">{partnerName}</p>
            </div>
            <div className="rounded-xl border border-rain-cloud/12 bg-rain-mist/35 p-3">
